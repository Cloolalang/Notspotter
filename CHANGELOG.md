...
- RXSS pulse controls now use consistent wording across all tiers: **Alert pulse** (enable toggle), **Alert pulse volume**, plus the existing duration/interval/frequency labels.
- Voice Test buttons now preview **voice only** — they no longer play the alert pulse/tone (or vibration) first. Pulse Test buttons still preview the pulse.
- Removed leftover RXSS panel descriptions (“Voice and alert tone”, shared-hint copy, section intros). Interval/duration clash warnings remain.
...
- "Passive signal thresholds and alert settings" panel was only visible while monitoring/testing was running. It's now always accessible inside **Passive mode monitoring** (its Test/preview buttons remain disabled while running, same as other settings panels).
...
- Introduced a two-level hierarchy of collapsible panels on the settings screen, grouped under four new top-level collapsible sections: **Passive mode monitoring**, **Active mode testing**, **General App settings**, and **Development area**.
- Moved "Monitoring options" and "Mock network state" into **Passive mode monitoring**.
- Moved "Ping settings" into **Active mode testing**.
- Moved "Settings profiles", "Global alert sound settings", and "VA — Voice announcement preferences" into **General App settings**.
- Moved "Carrier config (UE behavior) — research" into **Development area**.
...
- Renamed the "4G layer resilience" metric label to **"4G layers detected"** for clarity.
