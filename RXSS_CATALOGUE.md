...
| **12** | Limited service alt 4G | 4G/5G | Camp / SOS | Alternate | Visited operator | Active | Camped, not registered | Yes | **(1–6 + optional 14) or 20 or 26** — **14 not with 20/26**; **20 not with 26**; **not 13** | Emergency-only on visited-operator 4G/5G. Mock: `ALT_OPERATOR_4G` | Yes | Home 4G is **19** | **Entry:** `{home} home, {visited} visited, {tech}, visiting limited service` (**VA-4**) · **Exit:** silent · **30s:** (**VA-14**) when no overlay · Overlays: **VA-15** (6), **VA-12**/**VA-2** (20) · **VA-6** operator change · `limitedServiceVoiceEnabled`, `limitedServiceVoiceVolume`, `speakVisitingLimitedService` | **Camp signal pulses** or legacy **Two-tone** · `limitedServiceTierSoundEnabled`, `limitedServiceTierClickIntervalMs`, `limitedServiceToneVolume` · overlay **20** uses **RXSS 10** voice/tone |
...
| **19** | Limited service home 4G | 4G/5G | Camp / SOS | Home | Limited home | Suspended | Camped, not registered | Yes | **(1–6 + optional 14) or 20** — **14 not with 20**; **not 22** | Emergency-only on home-operator 4G/5G (e.g. SIM account suspended). Mock: `HOME_LIMITED_4G` | Yes | | **Entry / 30s:** `{operator}, {tech}, home limited service` (**VA-4** / **VA-14**) · reuse `limitedServiceVoice*` · `speakHomeLimitedService` | **Camp signal pulses** · reuse `limitedServiceTier*` with **12** |
| **20** | Limited service 4G no signal (alt/active) | 4G/5G | Camp / SOS overlay | Alternate or home | Follows base | Follows base†† | Transitory state | No | **12** (alt/active) **or 19** (home/susp) | No RSRP on **12** (active SIM) or **19** (home/suspended); **not 10**; **not 26**. Mock: `ALT_OPERATOR_4G` or `HOME_LIMITED_4G` weak RSRP | Yes | | **Entry / 30s / exit:** `{operator} [visited], {tech}, no signal` / `signal restored` plus home/visiting limited-service phrase (**VA-12** / **VA-2**) · shares **RXSS 10** controls | **Camp signal pulses** · `noSignalTierSoundEnabled`, `noSignalTierClickIntervalMs` |
| **21** | Limited home 2G no signal | 2G | Camp / SOS overlay | Home | Limited home | Suspended | Transitory state | No | **22** | Limited home 2G camp with no usable RX; **not** RXSS 15. Mock: `HOME_LIMITED_2G` weak RX | Yes | | **Entry / 30s / exit:** `{operator}, 2 G, no signal, home limited service` · shares **RXSS 15** / no-signal voice | **Camp signal pulses** · `g2NoSignalTierSoundEnabled`, `g2NoSignalTierClickIntervalMs` |
| **22** | Limited service home 2G | 2G | Camp / SOS | Home | Limited home | Suspended | Camped, not registered | Yes | **7 or 8 or 21** + **16 + 17 (always paired)**‡ — **21 not with 7/8**; **not 19** | Emergency-only on home-operator 2G (e.g. SIM account suspended). Mock: `HOME_LIMITED_2G` | Yes | Distinct from **11** (search) and **15** (in-service home 2G no signal) | **Entry / 30s:** `{operator}, 2 G, home limited service` (**VA-4** / **VA-14**) · overlay **8** uses **VA-18** signal low · `speakHomeLimitedService` | **Camp signal pulses** · reuse `limitedAlt2gTier*` with **13** |
...
| **31** | WiFi calling, no cellular signal | WLAN (VoWiFi) | Camp | Home | Full home | Active | In service (WLAN transport, no cellular RAT) | No | **not 0, 10, 11** | `ServiceState.getNetworkRegistrationInfoList()` reports a home-registered `TRANSPORT_TYPE_WLAN` entry (API 30+; falls back to `dataNetworkType == NETWORK_TYPE_IWLAN` below API 30) with no cellular RAT camped and no RSRP/RSRQ; debounced via `noSignalActive`. Mock: `WIFI_CALLING` | Yes | | **Entry / 30s:** “{op}, wifi calling, no cellular signal” · **Exit:** “{op}, cellular signal restored” · reuses **VA-1** / **VA-2** / **VA-12** triggers and `noSignalVoiceEnabled` / `noSignalVoiceVolume` | **Camp signal pulses** · own **RXSS 31** section in Passive Signal Settings with independent `wifiCallingTierSoundEnabled`, `wifiCallingTierClickIntervalMs`, `wifiCallingTierPulseDurationMs`; tone volume/frequency (`noSignalToneVolume` / `noSignalTierPulseFrequencyHz`) shared with the RXSS 10 group |
...
| `NO_SERVICE` | **0** | No |
| `SEARCHING_2G` | **11** | No |
| `HOME_5G` | **1–6** (+ **14**) | Yes (RSRP + RSRQ) |
| `HOME_5G_ENDC` | **1–6** (+ **14**) | Yes (RSRP + RSRQ) |
| `HOME_LIMITED_4G` | **19** (+ **1–6** / **20**) | Yes |
| `HOME_LIMITED_2G` | **22** (+ **7–8** / **21**) | Yes (RX level) |
| `ROAMING_4G` | **1–6** / **10** | Yes |
| `ROAMING_2G` | **7–8** / **15** | Yes (RX level) |
| `ROAMING_5G` | **1–6** / **10** | Yes |
| `ROAMING_5G_ENDC` | **1–6** / **10** | Yes |
| `WIFI_CALLING` | **31** | No |

Mock settings (`enabled`, `scenario`, `rsrpDbm`, `rsrqDb`, `voiceOnlyNoData`) are included in saved profiles — [SETTINGS_PROFILES.md](SETTINGS_PROFILES.md).
