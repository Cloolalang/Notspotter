...
data class CellularRadioMetrics(
    ...
    /**
     * "4G layers detected" — split into primary-channel and alternate-layer counts. See
     * [LteLayerResilienceReading]. Uses whatever the modem already reports with no extra
     * signal-quality floor, and reflects only currently *detected* layers — idle-mode neighbour
     * measurement is inherently limited by the serving cell's SIB4/SIB5 configuration (3GPP TS
     * 36.304), so this can under-count layers that physically exist but haven't been
